...
def main():
    ...
    if not os.path.exists(imdb_path):
        print(f"❌ ОШИБКА: Файл {imdb_path} не найден!")
    ...
    print(f"✓ IMDB загружен: {len(df_en):,} записей")
    ...
    if not os.path.exists(russian_path):
        print(f"⚠ Файл {russian_path} не найден!")
        print("Создаем тестовый русский датасет...")
        os.system('python create_russian_dataset.py')

        if not os.path.exists(russian_path):
            print("❌ Не удалось создать русский датасет")
            return

    df_ru = pd.read_csv(russian_path)
    print(f"✓ Русский датасет загружен: {len(df_ru):,} записей")
