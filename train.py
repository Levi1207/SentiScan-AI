...
def main():
    ...
    if os.path.exists(combined_path):
        ...
        print("✓ Используем объединенный датасет (EN + RU)")
    elif os.path.exists(imdb_path):
        dataset_path = imdb_path
        print("⚠ Используем только IMDB датасет (EN)")
    ...
    if empty_count > 0:
        print(f"⚠ Найдено {empty_count} пустых текстов после очистки, удаляем...")
        df = df[df['clean_review'].str.len() > 0]

    print(f"✓ Очистка завершена. Осталось {len(df):,} записей")
